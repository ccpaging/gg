...
// and prepares a DeviceContext for rendering onto that image.
func NewDeviceContext(width, height int) *DeviceContext {
	return NewDeviceContextForRGBA(image.NewRGBA(image.Rect(0, 0, width, height)))
}

// NewDeviceContextForImage copies the specified image into a new image.RGBA
// and prepares a DeviceContext for rendering onto that image.
func NewDeviceContextForImage(img image.Image) *DeviceContext {
	return NewDeviceContextForRGBA(imageToRGBA(img))
}

// NewDeviceContextForRGBA prepares a DeviceContext for rendering onto the specified image.
// No copy is made.
func NewDeviceContextForRGBA(img *image.RGBA) *DeviceContext {
	w := img.Bounds().Size().X
	h := img.Bounds().Size().Y
...
// The point will have been transformed by the DeviceContext's transformation matrix.
func (dc *DeviceContext) GetCurrentPoint() (Point, bool) {
	if dc.hasCurrent {
...
// Image returns the image that has been drawn by this DeviceContext.
func (dc *DeviceContext) Image() image.Image {
	return dc.img
}

// Width returns the width of the image in pixels.
func (dc *DeviceContext) Width() int {
	return dc.width
}
...
func (dc *DeviceContext) SetDash(dashes ...float64) {
	dc.dashes = dashes
}

// SetDashOffset sets the initial offset into the dash pattern to use when
// stroking dashed paths.
func (dc *DeviceContext) SetDashOffset(offset float64) {
	dc.dashOffset = offset
}

func (dc *DeviceContext) SetLineWidth(lineWidth float64) {
	dc.lineWidth = lineWidth
}

func (dc *DeviceContext) SetLineCap(lineCap LineCap) {
	dc.lineCap = lineCap
}

func (dc *DeviceContext) SetLineCapRound() {
	dc.lineCap = LineCapRound
}

func (dc *DeviceContext) SetLineCapButt() {
	dc.lineCap = LineCapButt
}

func (dc *DeviceContext) SetLineCapSquare() {
	dc.lineCap = LineCapSquare
}

func (dc *DeviceContext) SetLineJoin(lineJoin LineJoin) {
	dc.lineJoin = lineJoin
}

func (dc *DeviceContext) SetLineJoinRound() {
	dc.lineJoin = LineJoinRound
}

func (dc *DeviceContext) SetLineJoinBevel() {
	dc.lineJoin = LineJoinBevel
}

func (dc *DeviceContext) SetFillRule(fillRule FillRule) {
	dc.fillRule = fillRule
}

func (dc *DeviceContext) SetFillRuleWinding() {
	dc.fillRule = FillRuleWinding
}

func (dc *DeviceContext) SetFillRuleEvenOdd() {
	dc.fillRule = FillRuleEvenOdd
}

// Color Setters

func (dc *DeviceContext) setFillAndStrokeColor(c color.Color) {
	dc.color = c
...
func (dc *DeviceContext) SetFillStyle(pattern Pattern) {
...
func (dc *DeviceContext) SetStrokeStyle(pattern Pattern) {
	dc.strokePattern = pattern
}

// SetColor sets the current color(for both fill and stroke).
func (dc *DeviceContext) SetColor(c color.Color) {
...
func (dc *DeviceContext) SetHexColor(x string) {
...
// 255, inclusive.
func (dc *DeviceContext) SetRGBA255(r, g, b, a int) {
...
func (dc *DeviceContext) SetRGB255(r, g, b int) {
	dc.SetRGBA255(r, g, b, 255)
}

// SetRGBA sets the current color. r, g, b, a values should be between 0 and 1,
// inclusive.
func (dc *DeviceContext) SetRGBA(r, g, b, a float64) {
...
func (dc *DeviceContext) SetRGB(r, g, b float64) {
	dc.SetRGBA(r, g, b, 1)
}
...
// specified point.
func (dc *DeviceContext) MoveTo(x, y float64) {
	if dc.hasCurrent {
...
func (dc *DeviceContext) LineTo(x, y float64) {
	if !dc.hasCurrent {
		dc.MoveTo(x, y)
	} else {
		x, y = dc.TransformPoint(x, y)
...
// MoveTo(x1, y1)
func (dc *DeviceContext) QuadraticTo(x1, y1, x2, y2 float64) {
	if !dc.hasCurrent {
		dc.MoveTo(x1, y1)
	}
...
func (dc *DeviceContext) CubicTo(x1, y1, x2, y2, x3, y3 float64) {
	if !dc.hasCurrent {
		dc.MoveTo(x1, y1)
	}
...
func (dc *DeviceContext) ClosePath() {
	if dc.hasCurrent {
...
// operation.
func (dc *DeviceContext) ClearPath() {
	dc.strokePath.Clear()
	dc.fillPath.Clear()
	dc.hasCurrent = false
}
...
func (dc *DeviceContext) NewSubPath() {
	if dc.hasCurrent {
		dc.fillPath.Add1(dc.start.Fixed())
	}
	dc.hasCurrent = false
...
func (dc *DeviceContext) capper() raster.Capper {
	switch dc.lineCap {
	case LineCapButt:
...
func (dc *DeviceContext) joiner() raster.Joiner {
	switch dc.lineJoin {
	case LineJoinBevel:
...
func (dc *DeviceContext) stroke(painter raster.Painter) {
	path := dc.strokePath
	if len(dc.dashes) > 0 {
...
func (dc *DeviceContext) fill(painter raster.Painter) {
	path := dc.fillPath
	if dc.hasCurrent {
...
// operation.
func (dc *DeviceContext) StrokePreserve() {
	...
	if dc.mask == nil {
		if pattern, ok := dc.strokePattern.(*solidPattern); ok {
			...
			p := raster.NewRGBAPainter(dc.img)
			p.SetColor(pattern.color)
			painter = p
		}
	}
	if painter == nil {
		painter = newPatternPainter(dc.img, dc.mask, dc.strokePattern)
	}
	dc.stroke(painter)
}
...
func (dc *DeviceContext) FillPreserve() {
	...
	if painter == nil {
		painter = newPatternPainter(dc.img, dc.mask, dc.fillPattern)
	}
	dc.fill(painter)
}
...
func (dc *DeviceContext) ClipPreserve() {
...
// mask. It must be the same size as the DeviceContext, else an error is returned
// and the mask is unchanged.
func (dc *DeviceContext) SetMask(mask *image.Alpha) error {
	if mask.Bounds().Size() != dc.img.Bounds().Size() {
		return errors.New("mask size must match DeviceContext size")
	}
	dc.mask = mask
	return nil
}
...
// DeviceContext. This can be useful for advanced clipping operations where you first
// render the mask geometry and then use it as a mask.
func (dc *DeviceContext) AsMask() *image.Alpha {
	mask := image.NewAlpha(dc.img.Bounds())
	draw.Draw(mask, dc.img.Bounds(), dc.img, image.Point{}, draw.Src)
	return mask
}
...
func (dc *DeviceContext) InvertMask() {
	if dc.mask == nil {
		dc.mask = image.NewAlpha(dc.img.Bounds())
	} else {
...
func (dc *DeviceContext) Clip() {
	dc.ClipPreserve()
	dc.ClearPath()
}

// ResetClip clears the clipping region.
func (dc *DeviceContext) ResetClip() {
	dc.mask = nil
}
...
func (dc *DeviceContext) Clear() {
	src := image.NewUniform(dc.color)
	draw.Draw(dc.img, dc.img.Bounds(), src, image.Point{}, draw.Src)
}

// SetPixel sets the color of the specified pixel using the current color.
func (dc *DeviceContext) SetPixel(x, y int) {
	dc.img.Set(x, y, dc.color)
}
...
func (dc *DeviceContext) DrawPoint(x, y, r float64) {
	dc.Push()
...
func (dc *DeviceContext) DrawLine(x1, y1, x2, y2 float64) {
	dc.MoveTo(x1, y1)
	dc.LineTo(x2, y2)
}

func (dc *DeviceContext) DrawRectangle(x, y, w, h float64) {
	dc.NewSubPath()
	dc.MoveTo(x, y)
	dc.LineTo(x+w, y)
	dc.LineTo(x+w, y+h)
...
func (dc *DeviceContext) DrawRoundedRectangle(x, y, w, h, r float64) {
...
func (dc *DeviceContext) DrawEllipticalArc(x, y, rx, ry, angle1, angle2 float64) {
	const n = 16
	for i := 0; i < n; i++ {
		p1 := float64(i+0) / n
		p2 := float64(i+1) / n
...
func (dc *DeviceContext) DrawEllipse(x, y, rx, ry float64) {
	dc.NewSubPath()
	dc.DrawEllipticalArc(x, y, rx, ry, 0, 2*math.Pi)
	dc.ClosePath()
}

func (dc *DeviceContext) DrawArc(x, y, r, angle1, angle2 float64) {
	dc.DrawEllipticalArc(x, y, r, r, angle1, angle2)
}

func (dc *DeviceContext) DrawCircle(x, y, r float64) {
	dc.NewSubPath()
	dc.DrawEllipticalArc(x, y, r, r, 0, 2*math.Pi)
	dc.ClosePath()
}

func (dc *DeviceContext) DrawRegularPolygon(n int, x, y, r, rotation float64) {
...
func (dc *DeviceContext) DrawImageAnchored(img image.Image, x, y int, ax, ay float64) {
	s := img.Bounds().Size()
...
func (dc *DeviceContext) SetFontFace(fontFace font.Face) {
...
func (dc *DeviceContext) drawString(img *image.RGBA, s string, x, y float64) {
	d := &font.Drawer{
		Dst:  img,
		Src:  image.NewUniform(dc.color),
		Face: dc.fontFace,
...
func (dc *DeviceContext) DrawString(s string, x, y float64) {
...
func (dc *DeviceContext) DrawStringAnchored(s string, x, y, ax, ay float64) {
...
func (dc *DeviceContext) DrawStringWrapped(s string, x, y, ax, ay, width, lineSpacing float64, align Align) {
...
func (dc *DeviceContext) MeasureMultilineString(s string, lineSpacing float64) (width, height float64) {
	...
	for _, line := range lines {
		...
		currentWidth := float64(adv >> 6) // from gg.DeviceContext.MeasureString
...
func (dc *DeviceContext) MeasureString(s string) (w, h float64) {
	d := &font.Drawer{
		Face: dc.fontFace,
	}
	a := d.MeasureString(s)
...
// font face.
func (dc *DeviceContext) WordWrap(s string, w float64) []string {
...
func (dc *DeviceContext) Identity() {
	dc.matrix = Identity()
}

// Translate updates the current matrix with a translation.
func (dc *DeviceContext) Translate(x, y float64) {
	dc.matrix = dc.matrix.Translate(x, y)
}

// Scale updates the current matrix with a scaling factor.
// Scaling occurs about the origin.
func (dc *DeviceContext) Scale(x, y float64) {
...
func (dc *DeviceContext) ScaleAbout(sx, sy, x, y float64) {
	dc.Translate(x, y)
	dc.Scale(sx, sy)
	dc.Translate(-x, -y)
}
...
func (dc *DeviceContext) Shear(x, y float64) {
...
func (dc *DeviceContext) ShearAbout(sx, sy, x, y float64) {
	dc.Translate(x, y)
	dc.Shear(sx, sy)
	dc.Translate(-x, -y)
}
...
// Push saves the current state of the DeviceContext for later retrieval. These
// can be nested.
func (dc *DeviceContext) Push() {
	x := *dc
